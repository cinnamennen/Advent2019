...
class Computer:
    ...
    def get_operation(self) -> int:
        ...
        if debug:
            print(
                f"opcode was {op_code} with memory {self.memory[self.position:self.position + 4]} offset {self.relative_base}"
            )
...
def solve(data=None, data_in=None):
    if data_in is None:
        data_in = []
    c = make_computer(data)
    while data_in:
        c.data_in.put(data_in.pop(0))
    return c.run(), c


def make_computer(data):
    return Computer(get_input(data))


def main():
    print(solve(data_in=[2])[0])
